...
export function findDiagonalImbalances(
  ...
  const priceStep = candle.priceStep;
  ...
  if (
    ...
    throw new Error(
      "Price step must be finite and greater than 0"
    );
  }
  ...
  for (const level of candle.levels.values()) {
    ...
    const lower =
      candle.levels.get(lowerPrice);

    const upper =
      candle.levels.get(upperPrice);
...
export function findStackedImbalances(
  ...
  for (
    ...
    for (const imbalance of sorted) {
      ...
      const expectedPrice = previous
        ? getPriceLevel(
            previous.price + priceStep,
            priceStep
          )
        : undefined;
